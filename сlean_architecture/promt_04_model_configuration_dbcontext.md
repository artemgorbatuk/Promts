...
- Перед созданием конфигурационного файла (`ConfigurationEntityName.cs`) ИИ должен задать вопросы, необходимые для корректной конфигурации:
  - Какое точное имя таблицы (во множественном числе) и схема (если используется)?
  - Какие поля обязательные/необязательные (nullable) и какие ограничения длины для строк?
  - Какие поля должны храниться как `text`, а какие как ограниченная строка (`HasMaxLength`)?
  - Какие числовые поля требуют точности/масштаба (например, `numeric(7,2)`), и какие значения по умолчанию нужны (если нужны)?
  - Какие индексы нужны (уникальные/неуникальные), по каким колонкам и как назвать (`IX_...`)?
  - Какие связи есть (1:1, 1:N, N:N, самореферентные), какие FK обязательные/необязательные и какое поведение при удалении нужно (`Restrict`/`SetNull`/`Cascade`)?
  - Для N:N: нужно ли настраивать join-таблицу явно (имя, составной ключ, индексы) и в какой конфигурации это делаем?
